...
export async function runIsolatedPlaywrightProject({ projectName, runLabel }) {
  ...
  try {
    ...
    const frontend = spawnManaged(
      ...
      ['vite', '--host', '127.0.0.1', '--port', String(frontendPort)],
      {
        ...
        env: {
          ...
          ...(projectName === 'benchmark' ? { VITE_BENCHMARK_RAW_TRANSCRIPT: '1' } : {}),
        },
      },
    )
    ...
    const playwright = spawn('bunx', ['playwright', 'test', `--project=${projectName}`], {
      cwd: repoRoot,
      env: {
        ...process.env,
        BACKEND_PORT: String(backendPort),
